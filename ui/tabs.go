...
func tabBadgeWidth(badge string) float32 {
	...
	bw, _ := frameText().MeasureAt(badge, theme.Current().Typography.Caption.Size)
...
func (t *TabBar) layoutTabs() []tabExtent {
	...
	for i, tab := range t.Tabs {
		...
		tw, _ := frameText().MeasureAt(title, style.Size)
...
func (t *TabBar) paint(dl *render.DrawList, text *shape.Engine) {
	...
	for i, tab := range t.Tabs {
		...
		if i == t.hoverClose {
			...
			frameIcons().Draw(dl, "close", c, th.Foreground)
		} else if tab.Modified {
			frameIcons().Draw(dl, "circle", shrinkRect(c, 0.5), th.ForegroundMuted)
		} else if i == t.hoverTab || i == t.Active {
			frameIcons().Draw(dl, "close", c, th.ForegroundMuted)
		}
	}
