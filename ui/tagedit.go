...
func NewTagEdit(width float32, initial ...string) *TagEdit {
	...
	t.field = NewTextInput(TextFieldConfig{
		Placeholder: "Add tag...",
		Height:      chipH,
	})
...
func (t *TagEdit) chipSize(tag string) (w, h float32) {
	...
	tw, _ := frameText().MeasureAt(tag, style.Size)
...
func (t *TagEdit) paintChip(dl *render.DrawList, eng *shape.Engine, el *layout.Element, tag string) {
	...
	frameIcons().Draw(dl, "close", render.Rect{X: ix, Y: iy, W: iconSz, H: iconSz}, th.ForegroundMuted)
}
