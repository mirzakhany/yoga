...
func NewEnvironmentsPage() *EnvironmentsPage {
	...
	root := &ui.TreeNode{Label: "Environments", Data: "environments"}
	env.environments = ui.NewTree(root)
	env.environments.Loader = func(n *ui.TreeNode) []*ui.TreeNode {
		if n != root {
			return nil
		}
		return []*ui.TreeNode{
...
func (e *EnvironmentsPage) sideBar(c *ui.Ctx) ui.View {
	...
	return ui.Column(
		...
		ui.Row(
			...
			ui.Button("new", ui.Text("New")).Primary().IconStart("add"),
